...
namespace Antlr4.Runtime
{
    ...
    public class BufferedTokenStream : ITokenStream
    {
        ...
        /// EOF.
        /// </remarks>
        protected internal virtual int NextTokenOnChannel(int i, int[] channels)
        {
            Sync(i);
            if (i >= Size)
            {
            ...
            while(!channels.Contains(token.Channel))
            {
        ...
        /// </remarks>
        protected internal virtual int PreviousTokenOnChannel(int i, int[] channels)
        {
            Sync(i);
            if (i >= Size)
            {
            ...
            {
                IToken token = tokens[i];
                if (token.Type == TokenConstants.EOF || channels.Contains(token.Channel))
                {
                    return i;
                }
                i--;
        ...
        /// </summary>
        public virtual IList<IToken> GetHiddenTokensToRight(int tokenIndex, int[] channels)
        {
            LazyInit();
            ...
            int nextOnChannel = NextTokenOnChannel(tokenIndex + 1, new[]{Lexer.DefaultTokenChannel});
            int to;
            ...
            return FilterForChannel(from, to, channels);
        }
        ...
        {
            return GetHiddenTokensToRight(tokenIndex, new[] {-1});
        }
        ...
        /// , find any non default channel token.
        /// </summary>
        public virtual IList<IToken> GetHiddenTokensToLeft(int tokenIndex, int[] channels)
        {
            LazyInit();
            if (tokenIndex < 0 || tokenIndex >= tokens.Count)
            {
            ...
            int prevOnChannel = PreviousTokenOnChannel(tokenIndex - 1, new[]{Lexer.DefaultTokenChannel});
        ...
        {
            return GetHiddenTokensToLeft(tokenIndex, new[]{-1});
        }

        protected internal virtual IList<IToken> FilterForChannel(int from, int to, int[] channels)
        {
